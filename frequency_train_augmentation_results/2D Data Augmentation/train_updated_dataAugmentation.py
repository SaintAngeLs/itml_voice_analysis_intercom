...
class DataLoader:
    ...
    def __init__(self, spectrogram_dir, target_size=(128, 128), chunk_size=None):
        ...
        self.datagen = ImageDataGenerator(
            rotation_range=30, # Randomly rotate the image within the range of -30 to +30 degrees.
            width_shift_range=0.2,# Randomly shift the image horizontally (from -20% to +20% of the image width).
            height_shift_range=0.2,# Randomly shift the image vertically (from -20% to +20% of the image height).
            shear_range=0.2,# Apply shear transformation to the image at a random angle.
            zoom_range=0.2,# Randomly zoom in or out on the image within the range of 80% to 120% of the original size.
            horizontal_flip=True,# Flip the image horizontally.
            fill_mode='nearest'# fill_mode is not a transformation technique but a method to fill empty pixels created by applied transformation
        )
        ...
        for class_name in ['allowed', 'disallowed']:
            ...
            for file_name in os.listdir(class_dir):
                if file_name.endswith('_spectrogram.png'):
                    ...
                    image_array = self.datagen.random_transform(image_array)
...
def main():
    ...
    results_file = os.path.join(output_dir, "split_results_with_augmentation.json")
